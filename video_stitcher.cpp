...
MyVideoStitcher::MyVideoStitcher()
{
	...
	parallel_num_=sysconf(_SC_NPROCESSORS_ONLN);
}
...
int MyVideoStitcher::stitch( vector<VideoCapture> &captures, string &writer_file_name )
{
	...
	if(is_save_video_)
	{
		writer.open(writer_file_name, cv::VideoWriter::fourcc('D', 'I','V', '3'), 20, Size(dst.cols, dst.rows));
		writer.write(dst);
	}
...
int MyVideoStitcher::FindFeatures(vector<Mat> &src, vector<ImageFeatures> &features)
{
    ...
    for (int i = 0; i < num_images; ++i)
    {
        ...
        else
        {
            ...
            {
                work_scale_ = min(1.0, sqrt(work_megapix_ * 1e6 / full_img.size().area()));
                is_work_scale_set_ = true;
            }
        ...
        {
            seam_scale_ = min(1.0, sqrt(seam_megapix_ * 1e6 / full_img.size().area()));
...
int MyVideoStitcher::MatchImages(vector<ImageFeatures> &features, vector<MatchesInfo> &pairwise_matches)
{
    ...
    if (match_type_ == "affine")
        matcher = makePtr<AffineBestOf2NearestMatcher>(false, is_try_gpu_, match_conf_);
    else if (range_width_==-1)
        matcher = makePtr<BestOf2NearestMatcher>(is_try_gpu_, match_conf_);
    else
        matcher = makePtr<BestOf2NearestRangeMatcher>(range_width_, is_try_gpu_, match_conf_);
...
double MyVideoStitcher::GetViewAngle(vector<Mat> &src, vector<CameraParams> &cameras)
{
	...
	Rect result_roi = resultRoi(corners, sizes);//return the max rect
 ...
 */
int MyVideoStitcher::WarpForSeam(vector<Mat> &src, vector<CameraParams> &cameras, vector<UMat> &masks_warped, vector<UMat> &images_warped)
{
	// Warp images and their masks
	int num_images = static_cast<int>(src.size());
    vector<UMat> masks(num_images);
#ifdef HAVE_OPENCV_CUDAWARPING
    if (is_try_gpu_ && cuda::getCudaEnabledDeviceCount() > 0)
    {
        if (warp_type_ == "plane")
            warper_creator_ = makePtr<cv::PlaneWarperGpu>();
        else if (warp_type_ == "cylindrical")
            warper_creator_ = makePtr<cv::CylindricalWarperGpu>();
        else if (warp_type_ == "spherical")
            warper_creator_ = makePtr<cv::SphericalWarperGpu>();
    }
    else
#endif
    {
        if (warp_type_ == "plane")
            warper_creator_ = makePtr<cv::PlaneWarper>();
        else if (warp_type_ == "affine")
            warper_creator_ = makePtr<cv::AffineWarper>();
        else if (warp_type_ == "cylindrical")
            warper_creator_ = makePtr<cv::CylindricalWarper>();
        else if (warp_type_== "spherical")
            warper_creator_ = makePtr<cv::SphericalWarper>();
        else if (warp_type_ == "fisheye")
            warper_creator_ = makePtr<cv::FisheyeWarper>();
        else if (warp_type_ == "stereographic")
            warper_creator_ = makePtr<cv::StereographicWarper>();
        else if (warp_type_ == "compressedPlaneA2B1")
            warper_creator_ = makePtr<cv::CompressedRectilinearWarper>(2.0f, 1.0f);
        else if (warp_type_ == "compressedPlaneA1.5B1")
            warper_creator_ = makePtr<cv::CompressedRectilinearWarper>(1.5f, 1.0f);
        else if (warp_type_ == "compressedPlanePortraitA2B1")
            warper_creator_ = makePtr<cv::CompressedRectilinearPortraitWarper>(2.0f, 1.0f);
        else if (warp_type_ == "compressedPlanePortraitA1.5B1")
            warper_creator_ = makePtr<cv::CompressedRectilinearPortraitWarper>(1.5f, 1.0f);
        else if (warp_type_ == "paniniA2B1")
            warper_creator_ = makePtr<cv::PaniniWarper>(2.0f, 1.0f);
        else if (warp_type_ == "paniniA1.5B1")
            warper_creator_ = makePtr<cv::PaniniWarper>(1.5f, 1.0f);
        else if (warp_type_ == "paniniPortraitA2B1")
            warper_creator_ = makePtr<cv::PaniniPortraitWarper>(2.0f, 1.0f);
        else if (warp_type_ == "paniniPortraitA1.5B1")
            warper_creator_ = makePtr<cv::PaniniPortraitWarper>(1.5f, 1.0f);
        else if (warp_type_ == "mercator")
            warper_creator_ = makePtr<cv::MercatorWarper>();
        else if (warp_type_ == "transverseMercator")
            warper_creator_ = makePtr<cv::TransverseMercatorWarper>();
    }
	...
	float warp_scale = static_cast<float>(median_focal_len_ * seam_work_aspect_);
	...
	for (int i = 0; i < num_images; ++i)
	{
		Mat_<float> K;
		cameras[i].K().convertTo(K, CV_32F);
		float swa = (float)seam_work_aspect_;
		K(0,0) *= swa; K(0,2) *= swa;
		K(1,1) *= swa; K(1,2) *= swa;
		...
		UMat tmp_mask_warped, tmp_img_warped;
		Point tmp_corner;
 ...
 */
int MyVideoStitcher::FindWidestInpaintRange(UMat umask, int &x1, int &x2)
{
	Mat mask;
	mask = umask.getMat(ACCESS_READ).clone();
 ...
 *	计算接缝
 */
int MyVideoStitcher::FindSeam(vector<UMat> &images_warped, vector<UMat> &masks_warped)
{
 ...
 *	恢复原始图像大小
 */
int MyVideoStitcher::Rescale(vector<Mat> &src, vector<CameraParams> &cameras, vector<UMat> &seam_masks, vector<UMat> &images_masks)
{
	...
	int num_images = static_cast<int>(src.size());
	Mat tmp_mask, tmp_dilated_mask, tmp_seam_mask, tmp_image_mask;
	corners_.clear();
	sizes_.clear();
	for (int src_idx = 0, seam_idx = 0; src_idx < num_images; ++src_idx)
	{
		...
		// Size sz = sizes_[src_idx];
		// Rect roi = warper->warpRoi(sz, K, cameras_[src_idx].R);
        // corners_[src_idx] = roi.tl();
        // sizes_[src_idx] = roi.size();
		...
		{
			int x1, x2;
			UMat tmp_mask_warped1 = tmp_mask_warped.getUMat(ACCESS_READ).clone();
			FindWidestInpaintRange(tmp_mask_warped1, x1, x2);
...
int MyVideoStitcher::PrepareAPAP(vector<Mat> &src)
{
	...
	// 计算接缝
	vector<UMat> seamed_masks(num_images);
	vector<UMat> images_warped(num_images);
	vector<UMat> init_masks(num_images);
	for(int i = 0; i < num_images; i++)
	{
		...
		seamed_masks[i] = final_warped_masks_[i].getUMat(ACCESS_READ);
	}
	...
	// 曝光补偿
	vector<UMat> final_warped_mask_1;
	for(int i = 0; i < final_warped_masks_.size();++i)
	{
		final_warped_mask_1[i] = final_warped_masks_[i].getUMat(ACCESS_READ);
	}

	compensator_.createWeightMaps(corners_, images_warped, final_warped_mask_1, ec_weight_maps_);
	...
	vector<Mat> seamed_masks_1;
	for(int i = 0; i < num_images; i++)
	{
		final_blend_masks_[i] = final_warped_masks_[i] & seamed_masks[i].getMat(ACCESS_READ);
		seamed_masks_1[i] = seamed_masks[i].getMat(ACCESS_READ).clone();
	}
	blender_.createWeightMaps(dst_roi_, corners_, seamed_masks_1, blend_weight_maps_);
...
int MyVideoStitcher::PrepareClassical(vector<Mat> &src)
{
	...
	// 把摄像机参数和masks还原到正常大小
	LOGLN("\t~rescaling...");
	this->Rescale(src, cameras_, masks_warped, images_warped);
