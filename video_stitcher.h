...
public:
	...
	void setRangewidth(int range_width) {range_width_ = range_width;};
	void setEstimator(string estimator_type){estimator_type_ = estimator_type;};
...
private:
	...
	int WarpForSeam(vector<Mat> &src, vector<CameraParams> &cameras,
		vector<UMat> &masks_warped, vector<UMat> &images_warped);

	/*
	 * 计算接缝
	 */
	int FindSeam(vector<UMat> &images_warped, vector<UMat> &masks_warped);

	/*
	 *	把摄像机参数和masks还原到正常大小
	 */
	int Rescale(vector<Mat> &src, vector<CameraParams> &cameras, vector<UMat> &seam_masks ,vector<UMat> &images_masks);
	 ...
	 */
	int FindWidestInpaintRange(UMat mask, int &x1, int &x2);
	...
	bool is_work_scale_set_;
	bool is_seam_scale_set_;
