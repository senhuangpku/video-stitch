...
void MyExposureCompensator::createWeightMaps( const vector<Point> &corners, const vector<UMat> &images, 
	const vector<UMat> &masks, vector<Mat_<float>> &ec_maps )
{
	vector<pair<UMat,uchar> > level_masks;
...
void MyExposureCompensator::feed( const vector<Point> &corners, const vector<UMat> &images, vector<UMat> &masks )
{
	vector<pair<UMat,uchar> > level_masks;
	for (size_t i = 0; i < masks.size(); ++i)
		level_masks.push_back(make_pair(masks[i], 255));
